...
trait Otp
{
    ...
    public function sendOtp($request, $type = Null)
    {
        ...
        if (is_numeric($request->email)) {
            // $smsTemplete = SmsTemplate::where('type_id', 24)->where('is_active', 1)->first(); //registration templete
            $msg = "رمز التحقق الخاص بك لتسجيل الدخول في سندباد ستور هو ". $code.'%0aYour otp code for sindbad store login is '. $code;
            try{
                if($request->email == '1111'){
                    return true;
                }

...
trait SendSMS
{
    ...
    function sendSMS($to, $text,$to_name='',$user_email='',$order_tracking_number = '',$secret_code='',$giftcard ='')
    {
        ...
        $url = "https://tamimahsms.com/user/smspush.aspx?username=swt2022&password=mko741&phoneno=$to&message=$text&sender=Sindhbad&source=Sindhbad";
        // return $url;
        ...
        elseif (BusinessSetting::where('type', 'TamimahSMS')->first()->status == 1) {
            
            $url = "https://tamimahsms.com/user/smspush.aspx?username=mes2022&password=lkj123&phoneno=$to&message=$text&sender=Sindhbad&source=Sindhbad";
            $cURLConnection = curl_init();
            curl_setopt($cURLConnection, CURLOPT_URL,$url);
            curl_setopt($cURLConnection, CURLOPT_RETURNTRANSFER, true);
            $response = curl_exec($cURLConnection);
